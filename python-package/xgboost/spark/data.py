...
def cache_partitions(
    ...
    for part in iterator:
        ...
        sum += difference
        get_logger("XGBoost-PySpark").warning("Copied data from JVM to Python takes {}: ".format(round(difference, 2)))
...
def create_dmatrix_from_partitions(  # pylint: disable=too-many-arguments
    ...
    start = time.time()
    get_logger("XGBoost-PySpark").warning("Begin to cache all partitions")
