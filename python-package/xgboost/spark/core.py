...
class _SparkXGBEstimator(Estimator, _SparkXGBParams, MLReadable, MLWritable):
    ...
    def _get_tracker_args(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        if launch_tracker_on_driver:
            ...
            if tracker.tracker_host is None:
            ...
            if self.isDefined(self.tracker):
                tracker = self.getOrDefault(self.tracker)
                assert isinstance(tracker, Config)
                if tracker.tracker_host is not None:
                    raise ValueError(
                        f"You must enable launch_tracker_on_driver to use "
                        f"tracker host: {tracker.tracker_host}"
                    )
        ...
        def _train_booster(
            ...
            if context.partitionId() == 0:
                if not launch_tracker_on_driver:
                    ...
                    _tracker.tracker_host = _get_host_ip(context)
...
class _SparkXGBSharedReadWrite:
    ...
    def saveMetadata(
        ...
        if tracker_conf is not None:
            extraMetadata["tracker_conf"] = tracker_conf.to_dict()
    ...
    def loadMetadataAndInstance(
        ...
        if "tracker_conf" in metadata:
            pyspark_xgb.set_tracker(Config.from_dict(metadata["tracker_conf"]))
