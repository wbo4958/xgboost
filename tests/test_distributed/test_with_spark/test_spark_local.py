...
class XgboostLocalTest(SparkTestCase):
    ...
    def test_tracker(self):
        classifier = SparkXGBClassifier(
            ...
            tracker=Config(tracker_host="192.168.1.32", tracker_port=59981),
        )
        ...
        classifier = SparkXGBClassifier(
            ...
            tracker=Config(tracker_host="127.0.0.1", tracker_port=58892),
        )
        with pytest.raises(
        ...
        classifier = SparkXGBClassifier(
            launch_tracker_on_driver=True,
            tracker=Config(tracker_host="127.0.0.1", tracker_port=58893),
            num_workers=2,
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            classifier = SparkXGBClassifier(
                launch_tracker_on_driver=True,
                tracker=Config(tracker_host="127.0.0.1", tracker_port=58894),
                num_workers=1,
                n_estimators=1,
            )

            def check_tracker(tracker: Config) -> None:
                assert tracker.tracker_host == "127.0.0.1"
